...
console.log(`${player.name} stands in front of a dark haunted house.`);
console.log(`${player.name} hears a crying baby sound and coming from inside the house.`);
let enterHouse = READLINE.question("Will you enter the house? (yes or no) ");
if(enterHouse == "yes" || enterHouse == "yes") {
  player.courage++;
  console.log(`${player.name} prepares their ${player.weapon} and opens the door. ${player.name} finds a baby , the face is cloaked by a black mask. the a lioght shines bright from beghind the figure revealing a fat spider looking down at ${player.name}`);
 console.log(`${player.name} tries to run away. ${player.name} turns and runs for the road but get grabbed back immediatly, getting dragged into the light, then it goes dark. ${player.name} looks for another way out. `)
  let ans = READLINE.question("there are two ways, fight the spider, or run for the door. what will you do? ");
